...
## Current state (as of this note, 2026-08-11)
...
- **ESP32 pointer never tested against real hardware** — `esp32=false` in
  `/status.json` during the Jetson run; `device_scanner.py` didn't find a
  real ESP32 on the LAN (only found the gateway and a few unidentified
  hosts). Pointer-move code path (`pointer_mapper.py`, `/proxy_servo`)
